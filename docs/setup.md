...
- ✅ Configure notifications using a simple fixed window (08:30–12:00)
- ✅ Point cron to `scripts/send_simple_notification.py`
...
# Add these lines to send simple reminders during the window only:
30 8 * * * cd /path/to/owlgorithm && ./duolingo_env/bin/python scripts/send_simple_notification.py >> logs/automation.log 2>&1
0,30 9-11 * * * cd /path/to/owlgorithm && ./duolingo_env/bin/python scripts/send_simple_notification.py >> logs/automation.log 2>&1
0 12 * * * cd /path/to/owlgorithm && ./duolingo_env/bin/python scripts/send_simple_notification.py >> logs/automation.log 2>&1
```
...
Notifications are now sent on a simple fixed window via cron. To change the window, adjust the cron lines generated by `scripts/setup_cron.py` or edit your crontab manually to new times.

### Custom Progress Reports
Notification content is a simple reminder sent by `scripts/send_simple_notification.py`. You can customize the title/message there.
...
- Content is a short reminder message
- No throttling, no dependency on scraping or progress state

## 🎉 Success!

Once configured, your tracker will:
- ✅ Scrape and update your progress report on demand (via `scripts/daily_update.py`)
- ✅ Send simple reminders during the morning window
